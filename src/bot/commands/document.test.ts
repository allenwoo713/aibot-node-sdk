...
import { parseDocumentCommand, handleDocumentCommand } from './document';

describe('parseDocumentCommand', () => {
  it('parses /文档 with docid argument', () => {
    const result = parseDocumentCommand('/文档 doc_123456');
    expect(result).toEqual({ type: 'document', arg: 'doc_123456' });
  });

  it('parses /文档 with URL argument', () => {
    const result = parseDocumentCommand('/文档 https://doc.weixin.qq.com/d/xxx');
    expect(result).toEqual({ type: 'document', arg: 'https://doc.weixin.qq.com/d/xxx' });
  });

  it('parses /文档 with extra whitespace', () => {
    const result = parseDocumentCommand('  /文档   doc_123  ');
    expect(result).toEqual({ type: 'document', arg: 'doc_123' });
  });

  it('returns document command with empty arg for missing argument', () => {
    const result = parseDocumentCommand('/文档');
    expect(result).toEqual({ type: 'document', arg: '' });
  });

  it('returns null for non-command text', () => {
    expect(parseDocumentCommand('Hello bot')).toBeNull();
    expect(parseDocumentCommand('/文档列表')).toBeNull();
    expect(parseDocumentCommand('/日程 创建 meeting')).toBeNull();
  });
});
...
describe('handleDocumentCommand', () => {
  ...
  it('returns format error for invalid URL', async () => {
    const reply = await handleDocumentCommand('https://bad.example.com/doc', createMockApiClient(), createMockAdapter(), 'internal', { maxInputTokens: 100 }, createMockLogger());
    expect(reply).toContain('文档链接格式不正确');
  });
